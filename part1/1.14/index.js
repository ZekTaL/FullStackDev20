...
const App = (props) => {
  // save clicks of each button to own state
	...
	const setToSelected = newValue => {
		...
		setVoted(points[newValue])
		}
	const AddPoint = newValue => {setVoted(points[newValue] += 1)}
	
	let idxMax = points.indexOf(Math.max(...points))
	...
	return (
		<div>
			...
			<h3>{props.anecdotes[idxMax]}</h3>
			<h4>Current Votes: {points[idxMax]}</h4>
		</div>
  )
}
